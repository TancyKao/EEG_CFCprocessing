...
The pipeline runs in three stages:
    ...
    3. Exports the parameters of these 'spindle' events.
...
Output:
    1. A new annotation file per subject, with individual marked spindles labelled by
       ...
       'spindle'.
    2. A csv output per subject with the spindle parameters for that recording.
    3. A group-level summary dataset of spindle parameters per subject.
...
# tag::Spindle_function[]

### General Parameters
evt_name = 'spindle'
method = ['Moelle2011'] #other options 'Moelle2011' 'Lacourse2018','Ferarelli2011', 'FASST','Ray2015', 'Wamsley2012'
# end::Spindle_function[]


# tag::par_function[]
cycle_idx = None #[1,2,3,4,5] # cycle index(es), starting at 1 ([None] = ignore cycles)
s_freq = 500 # sampling frequency of recordings
stage = ['NREM2'] # stages: NREM1, NREM2, NREM3, REM, Wake
reject_artf=['Arousal'] # if NREM3, remove 'Artefact'

# end::par_function[]


#chan = ['E20','E21', 'E22']#['E20','E21','E22']   #,'[C4 - A1]','[O1 - A2]', '[O2 - A1]']  # channels e.g. ['F3','C3', etc.]
...
# tag::freq_function[]
## spindle
frequency = (12, 15) # frequency limits of events, in Hz
duration = (0.5, 3) # duration limits of events, in secs
# end::freq_function[]
...
### spindle detection has issue?? ####
# '''
# SPINDLES
# '''

for chan in chan:
# # # Step 1. Detect spindles
    whale_it(in_dir, method, chan, rater, cat, stage, ref_chan, grp_name, 
                frequency, duration, polar, part, visit)


    # # Step 2. Extract spindle parameters 
